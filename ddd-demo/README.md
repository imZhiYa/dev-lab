...
> 与知识库 DDD 系列文章（ddd-00 ~ ddd-06，发布于 [tech-knowledge-docs · docs/11-ddd](https://github.com/imZhiYa/tech-knowledge-docs/tree/main/docs/11-ddd)）配套的代码实验。
...
| EX-02 | 防腐层（ACL）契约翻译：字段搬移、脏数据丢弃计数、本地模型隔离 | ddd-02 | 10 条 |
| EX-03 | 架构边界守护：ArchUnit 把"领域不依赖框架/上下文隔离"编译成测试 | ddd-02/03/04 | 5 条规则 |
| EX-04 | 读路径全链路：特征→策略视图→召回→排序→领域策略→曝光，含 4 种降级 | ddd-04 | 10 条 |
